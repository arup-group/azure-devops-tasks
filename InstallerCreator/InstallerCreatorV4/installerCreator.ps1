...
try {
   ...
   c:\tools\msys64\usr\bin\env MSYSTEM=MINGW64 /bin/bash -l -c "cd $linuxDir/oasys-combined/gsa-assembler/$installerDirectoryName-$majorNumber.$minorNumber && make clean all CLIENTID=$clientId CLIENTSECRET=$clientSecret 2>errors.log && if grep -iq 'error' errors.log; then cat errors.log && exit 1; fi"
} finally {
